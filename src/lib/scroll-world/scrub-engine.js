/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Vendored from https://github.com/oso95/scroll-world (MIT),
   skills/scroll-world/references/scrub-engine.js. Keep the diff small so
   upstream changes stay easy to merge. Local modifications, all at the bottom
   of mountScrollWorld():

     1. `mountScrollWorld` returns a `destroy()` that stops the rAF loop,
        removes the window listeners, empties the container and drops the
        injected <style>. Next.js keeps the document alive across client-side
...
function mountScrollWorld(container, config) {
  ...
  // Local addition — see the header. Returns the page to the state it was in
  // before mounting, so a client-side route change doesn't leave the scrub
  // loop running or the engine's global `html,body` rules applied.
  return function destroy() {
    ...
    // Object URLs are per-blob and die with the <video>; revoke them anyway so
    // the decoded clips are freed at once instead of at the next GC.
    SEGMENTS.forEach(s => { if (s.video) { try { URL.revokeObjectURL(s.video.src); } catch (e) {} } });
    container.innerHTML = '';
    container.classList.remove('sw-root');
    const css = document.getElementById('sw-css');
    if (css) css.remove();
  };
