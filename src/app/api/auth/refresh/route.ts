  ...
  try {
    ...
    if ("error" in result) {
      ...
      if (result.error === "reuse_detected") {
        await logAudit({ action: "auth.refresh_reuse_detected", ipAddress: clientIp(req.headers) }).catch(() => {});
      }
    ...
    return NextResponse.json({ error: "Layanan sedang tidak tersedia" }, { status: 503 });
  }
}
